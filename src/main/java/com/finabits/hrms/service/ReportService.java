...
import com.sitegenius.hrms.repository.SalaryRepository;
...
public class ReportService {
        ...
        for (User emp : employees) {
            ...
            for (LeaveTypeConfig lt : leaveTypes) {
                ...
                double usedForType = leaveRepository.findAll().stream()
                        .filter(l -> l.getUser().getId().equals(emp.getId()))
                        .filter(l -> l.getStatus() == com.sitegenius.hrms.enums.LeaveStatus.APPROVED)
            ...
            double unpaid = leaveRepository.findAll().stream()
                    .filter(l -> l.getUser().getId().equals(emp.getId()))
                    .filter(l -> l.getStatus() == com.sitegenius.hrms.enums.LeaveStatus.APPROVED)
                    .filter(l -> l.getLeaveType() == com.sitegenius.hrms.enums.LeaveType.UNPAID
                            || l.getLeaveType() == com.sitegenius.hrms.enums.LeaveType.MIXED)
    ...
    public byte[] generateEmployeeLeaveReport(Long userId, int year) throws IOException {
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new com.sitegenius.hrms.exception.ResourceNotFoundException("Employee not found"));
